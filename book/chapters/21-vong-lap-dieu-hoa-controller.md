# Chương 21 — Vòng lặp điều hòa và bộ điều khiển (controller) Pattern
...
Đây là bước chuyển từ **Quan sát thụ động (Passive Observation)** sang **Tự điều hòa chủ động (Active Self-Healing)**. Trái tim của mọi nền tảng hạ tầng đám mây hiện đại — từ Kubernetes bộ điều khiển (controller), Nomad, Terraform đến ArgoCD — đều vận hành dựa trên một mẫu kiến trúc cốt lõi: **Vòng lặp điều hòa (điều hòa trạng thái (reconciliation) Loop)** được điều phối bởi **bộ điều khiển (controller) Pattern**.
...
Ngược lại, mô hình **Level-Triggered** không quan tâm quá khứ đã xảy ra bao nhiêu lần chuyển trạng thái hay bao nhiêu thông điệp bị thất lạc. Ở mỗi chu kỳ, bộ điều khiển (controller) chỉ quan sát **Trạng thái thực tế (trạng thái thực tế)** hiện hành và so sánh với **Trạng thái mong muốn (trạng thái mong muốn)**:
...
Ngay cả khi bộ điều khiển (controller) bị tắt đột ngột lúc đang khởi động bản sao thứ hai, ở lần thức dậy kế tiếp, nó lại tiếp tục so sánh và nhận ra sai lệch vẫn còn (hiện có 2 bản sao, vẫn thiếu 1). Nó lặp lại hành động cho đến khi sai lệch triệt tiêu hoàn toàn. Đặc tính này gọi là **sự hội tụ trạng thái (State Convergence)**.
...
| **4** | **Requeue** | Xếp lịch kiểm tra lại hoặc lùi bước (backoff) nếu gặp lỗi. |
...
Một sai lầm thường gặp của lập trình viên Go là sử dụng ngay một unbuffered kênh truyền (`chan string`) để làm hàng đợi cho bộ điều khiển (controller). Trên môi trường môi trường vận hành, kênh truyền thô nhanh chóng bộc lộ ba lỗ hổng nghiêm trọng:

1. **Thiếu khả năng gộp trùng (Deduplication):** Khi một dịch vụ chập chờn, 50 sự kiện báo lỗi liên tiếp có thể dồn về trong một giây. Nếu đẩy cả 50 item vào kênh truyền, worker sẽ chạy 50 lần reconcile hoàn toàn trùng lặp, gây lãng phí tài nguyên vô ích.
...
3. **Bão thử lại (Retry Storm):** Nếu tài nguyên đích bị sập hoàn toàn, hàm `Act` sẽ liên tục trả về lỗi. Đẩy lại kênh truyền ngay lập tức sẽ khiến worker pool quay cuồng trong vòng lặp thử lại vô tận (spin-lock), vắt kiệt CPU và đánh sập chính dịch vụ đang hấp hối.

Để giải quyết triệt để ba bài toán trên, ta thiết kế cấu trúc `WorkQueue` chuyên dụng:

~~~go
type hàng đợi công việc (workqueue) struct {
	mu           sync.Mutex
	cond         *sync.Cond
	queue        []string
...
~~~go
func (q *hàng đợi công việc (workqueue)) Add(item string) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
~~~go
func (q *hàng đợi công việc (workqueue)) Done(item string) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
Khi `Reconcile` thất bại, thay vì đưa key trở lại hàng đợi ngay lập tức, ta áp dụng thuật toán **Exponential lùi bước (backoff)** thông qua phương thức `AddRateLimited`:

~~~go
func (q *hàng đợi công việc (workqueue)) AddRateLimited(item string) bool {
	q.mu.Lock()
	defer q.mu.Unlock()
	...
	if failures > q.cấu hình.MaxRetries {
		return false // Vượt quá số lần thử lại tối đa
	}

	// Chờ tăng gấp đôi: base * 2^(failures-1)
	multiplier := 1 << (failures - 1)
	delay := q.cấu hình.BaseDelay * time.Duration(multiplier)
	if delay > q.cấu hình.MaxDelay {
		delay = q.cấu hình.MaxDelay
	}
...
Nếu lần thử đầu tiên thất bại sau 50ms, lần kế tiếp sẽ diễn ra sau 100ms, rồi 200ms, 400ms, cho đến khi chạm trần `MaxDelay`. Khoảng thời gian giãn cách này tạo điều kiện cho hạ tầng mạng hoặc cơ sở dữ liệu có đủ thời gian tự hồi phục, ngăn ngừa triệt để hiện tượng bão retry. Khi đợt reconcile thành công, bộ điều khiển (controller) gọi `q.Forget(item)` để xóa bộ đếm thất bại, sẵn sàng cho các chu kỳ trong tương lai.
...
## Tắt nguồn mềm mại cho bộ điều khiển (controller)

bộ điều khiển (controller) quản lý nhiều worker goroutine chạy ngầm liên tục. Khi nhận tín hiệu dừng tiến trình (`SIGTERM` hoặc hủy context cha), toàn bộ worker phải hoàn tất các công việc đang dở dang trước khi trả quyền kiểm soát:

~~~go
func (c *bộ điều khiển (controller)) Run(ctx context.Context) error {
...
Phương thức `queue.ShutDown()` giải phóng các goroutine đang bị khối lệnh ở lệnh `cond.Wait()`. Vòng lặp `for c.processNextWorkItem(ctx)` nhận được tín hiệu `shutdown == true` khi hàng đợi đã cạn, kết thúc vòng lặp và thông báo qua `wg.Done()`. Quá trình này bảo đảm không có thao tác ghi dữ liệu nào bị đứt gánh giữa chừng.

## Thực hành Lab: Kiểm chứng bộ điều khiển (controller) và hàng đợi công việc (workqueue)

Toàn bộ kiến trúc trên được đóng gói tại thư mục thực hành `labs/part21-reconciliation-controller`:

~~~powershell
cd labs/part21-điều hòa trạng thái (reconciliation)-bộ điều khiển (controller)
...
- `TestController_HealFailure_RateLimitedBackoff`: Tự động lùi lịch thử lại bằng exponential lùi bước (backoff) khi hành động chữa lành bị lỗi.

## Bước phát triển tiếp theo

Hiểu và làm chủ bộ điều khiển (controller) Pattern là bước ngoặt đưa kỹ sư Go từ vai trò người xây dựng công cụ đơn lẻ trở thành kiến trúc sư của các hệ thống tự trị (*autonomous control systems*). Bằng cách kết hợp khả năng thu thập tín hiệu của `opsprobe` (Chương 20) với cơ chế điều hòa cấp độ mức của bộ điều khiển (controller) (Chương 21), hệ thống không chỉ biết nói cho ta biết nó đang đau ở đâu, mà còn có thể tự chữa lành vết thương trước khi người dùng kịp nhận ra.

Đây chính là nền tảng trực tiếp để ta mở rộng sang việc xây dựng các Custom bộ điều khiển (controller) và Operator tiêu chuẩn trong hệ sinh thái Kubernetes bằng `client-go` ở các chương chuyên sâu tiếp theo.

@references
1. Kubernetes Authors. bộ điều khiển (controller) pattern and declarative state management. kubernetes.io/docs/concepts/architecture/bộ điều khiển (controller)/
2. B. Grant. Declarative application management in Kubernetes. github.com/kubernetes/community
3. Kubernetes Authors. Package `client-go/util/workqueue` reference and design. pkg.go.dev/k8s.io/client-go/util/hàng đợi công việc (workqueue)
4. S. Newman. Building các dịch vụ vi mô: Designing Fine-Grained Systems and Self-Healing Architecture. O'Reilly Media.
5. Go Team. Advanced Go Concurrency Patterns: Pipelines and hủy thực thi. go.dev/blog/pipelines
