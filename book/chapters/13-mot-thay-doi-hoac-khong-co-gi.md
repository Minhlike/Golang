...
if _, err := db.ExecContext(ctx,
	"INSERT INTO check_events(check_id, action) các giá trị (?, ?)",
	checkID, "disabled",
); err != nil {
	return err
}
...
Nhưng hai lời gọi độc lập là hai boundary độc lập. Nếu `UPDATE` thành công còn `INSERT` lỗi vì constraint, hàm trả error thật, song database vẫn giữ điểm cuối ở trạng thái tắt. bên gọi có lý do để thử lại; người điều tra có một state không có nguyên nhân. Không có dòng `if` nào quay ngược câu lệnh đầu chỉ vì câu lệnh sau thất bại.

Transaction là cách nói với database rằng các câu lệnh này thuộc về một đơn vị: hoặc tất cả được commit, hoặc không câu lệnh nào trong đơn vị được giữ lại. Nó không phải bùa chú cho mọi vấn đề consistency; nó chỉ làm lời hứa hẹp nhưng cực quan trọng ấy tại boundary database.
...
> **Dừng để dự đoán:** nếu câu lệnh ghi event bị từ chối, test nào chứng minh điểm cuối vẫn `enabled`? Chỉ assert hàm trả error chưa đủ; state sau lỗi mới là điều contract phải khóa lại.
...
Đừng mở `fixed/` trước lượt tự làm đầu tiên. Contract đã nói rõ điều gì phải đúng, còn thiết kế thuộc về anh: transaction được mở ở đâu, cleanup đặt thế nào, câu lệnh nào phải chạy qua `tx` thay vì `db`, và `RowsAffected` được dùng ra sao để phân biệt không tìm thấy check với cập nhật thành công.
...
Ví dụ này chỉ minh họa boundary. Nó không chép sẵn `SetMaxOpenConns(50)` vì một con số đẹp không phải policy. `SetMaxOpenConns` giới hạn pool; khi mọi connection đang bận, yêu cầu sau phải chờ. Nếu mã nguồn đang giữ transaction lại cần một connection khác, giới hạn đó còn có thể tạo deadlock. Chọn giới hạn dựa trên concurrency thực, khả năng database, query time, connection budget và dữ liệu `DB.Stats`, rồi đo ở môi trường gần tải công việc. Đó là measured behavior; Go specification không hứa một throughput hay queue time nào.

hủy thực thi cũng dừng ở boundary driver. `QueryContext` và `BeginTx` nhận context, nhưng package `database/sql` ghi rõ driver không hỗ trợ context hủy thực thi có thể chỉ return sau khi query hoàn tất. Context vẫn phải được truyền; đồng thời, thời hạn xử lý (deadline) thực sự của query cần được kiểm chứng với đúng driver và database trước khi đưa vào SLO.
...
Đoạn mã nguồn này chưa nói gì về isolation: list có thể thấy row nào khi concurrent writer đang chạy là policy của transaction và database. Nó chỉ chốt vòng đời ở phía Go: query đã mượn resource thì hàm phải trả nó lại. Đây là cùng một kiểu trách nhiệm ta đã gặp với phản hồi body ở Chương 11 và listener ở Chương 12, chỉ ở một boundary khác.
...
Quay lại opening sự cố vận hành: phản hồi có thể mất *sau* `Commit`. bên gọi nhìn thấy lỗi mạng nhưng database đã đổi state. Transaction đã giữ lời hứa hẹp của nó: `UPDATE` và `INSERT` cùng tồn tại hoặc cùng không tồn tại. Nó chưa trả lời lần gọi lại có phải là cùng thao tác hay một yêu cầu mới.
...
Khi requirement nói client có thể retry cùng một thao tác mà chỉ được tạo một hiệu ứng, API cần identity cho thao tác đó: chẳng hạn idempotency key được bên gọi giữ lại qua lần gửi lại. Service cần xác định key đã được xử lý và trả kết quả phù hợp, thường bằng unique constraint hoặc record idempotency trong cùng database transaction. Chi tiết phản hồi khi key trùng, thời gian lưu key và side effect nào thuộc cùng thao tác đều là phần của điều API hứa. Chúng không tự xuất hiện từ `BeginTx`.

> **Bài suy luận ngắn.** Một bên gọi hết thời hạn (timeout) sau `Commit` rồi gọi lại `Disable` mà không gửi thao tác identity. State `enabled=false` nói được gì, và audit count nói được gì? Đáp án: bên gọi không thể phân biệt “lần trước của chính tôi đã commit” với “một actor khác đã tắt check”; count hai cũng không cho biết hai lần gọi có phải cùng ý định. Nếu distinction đó quan trọng, retry phải có contract riêng.

Tương tự, transaction không biến mọi concurrent thao tác thành tuần tự. `sql.TxOptions` cho phép bên gọi yêu cầu isolation; khi bên gọi yêu cầu non-default isolation level mà driver không hỗ trợ, `BeginTx` trả error. Database và driver thực tế vẫn quyết định các anomaly, lock và latency có ý nghĩa gì, nên trước khi dùng isolation để bảo vệ một invariant thật, đọc tài liệu database đang chạy, viết test cạnh tranh cho invariant đó và quan sát lỗi/lock/latency. Ở chương này, ta chỉ khóa một contract nhỏ có thể chứng minh cục bộ: event lỗi thì update không được tồn tại.

Schema cũng là state. Khi service có record thật, `CREATE TABLE` trong test không còn là cách đưa thay đổi ra môi trường vận hành. Migration cần thứ tự, người sở hữu, khả năng kiểm tra và kế hoạch rollback hoặc forward-fix. Cache cũng cần source of truth rõ ràng: nếu cache giữ `enabled=true` sau một commit tắt check, bên gọi vẫn đang thấy câu chuyện cũ. Những boundary đó sẽ được mở tiếp sau khi nền transaction đã đủ chắc.
...
@references
1. Go Team. Package `database/sql`: `DB`, `Tx`, `Rows`, context hủy thực thi và pool behavior. pkg.go.dev/database/sql
