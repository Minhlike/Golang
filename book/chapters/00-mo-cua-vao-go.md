...
## Bóc tách cú pháp nguyên tử (mã nguồn Anatomy)
...
1. **`package main`**:
   - `package`: Từ khóa (từ khóa) bắt buộc mở đầu mọi tệp nguồn Go. Nó khai báo rằng các định danh trong tệp này thuộc về một gói mã nguồn có tên là `main`.
   - `main`: Đây là tên gói đặc biệt được quy ước trong Go Specification. Khi một gói có tên là `main`, compiler hiểu rằng nó cần tạo ra một **tệp thực thi độc lập (executable binary)**, thay vì một thư viện dùng chung cho các dự án khác.

2. **`import "fmt"`**:
   - `import`: Từ khóa nạp gói thư viện bên ngoài vào tệp hiện tại.
   - `"fmt"`: Tên gói thư viện chuẩn (Standard Library) chuyên trách định dạng nhập xuất dữ liệu (Format I/O). Đường dẫn gói được đặt trong cặp dấu ngoặc kép.

3. **`func main() { ... }`**:
   - `func`: Từ khóa định nghĩa một hàm.
   - `main`: Tên hàm khởi điểm (entry point) của toàn bộ chương trình. Khi hệ điều hành khởi chạy tệp thực thi, chỉ lệnh đầu tiên của CPU sẽ nhảy thẳng vào thân hàm `main` này. Một hàm `main` khởi điểm chuẩn không nhận tham số và không trả về giá trị trực tiếp.
   - `()`: Cặp ngoặc đơn chứa danh sách tham số (ở đây để trống vì hàm không nhận tham số).
   - `{ ... }`: Khối lệnh (khối lệnh) bao bọc thân hàm. Mọi câu lệnh bên trong cặp ngoặc nhọn này sẽ được thực thi tuần tự từ trên xuống dưới.

4. **`fmt.Println(...)`**:
   - `fmt`: Tên của gói thư viện mà ta đã nạp ở phần `import`.
   - `.`: Toán tử truy cập định danh được xuất khẩu (exported tên định danh). Trong Go, mọi hàm hoặc biến bắt đầu bằng chữ cái in hoa (như `Println`) đều là công khai để các gói khác sử dụng.
...
| :--- | :--- | :--- |
| `go run main.go` | Biên dịch mã nguồn thành tệp nhị phân tạm thời trong bộ nhớ cache rồi chạy ngay lập tức. Sau khi chạy xong, tệp tạm sẽ được dọn dẹp. | Thử nghiệm nhanh, viết mã thử nghiệm, kiểm tra logic cục bộ. |
| `go build main.go` | Biên dịch toàn bộ mã nguồn thành một tệp thực thi mã máy độc lập (executable binary, ví dụ `main.exe` trên Windows hoặc `main` trên Linux). | Triển khai lên máy chủ, đóng gói container, chuyển giao sản phẩm. |

Tệp thực thi do `go build` tạo ra là mã máy thuần túy (native machine mã nguồn). Nó không cần máy ảo (như JVM trong Java) và không cần trình thông dịch (như Python). Bạn có thể sao chép tệp nhị phân đó sang một máy chủ Linux sạch sẽ không cài Go và nó vẫn chạy hoàn hảo.
...
@figure Vòng lặp phản hồi giả thuyết - đo đạc - sửa mã nguồn. Compiler không phải là chướng ngại vật; compiler là người cộng tác phản hồi nhanh nhất cho dự đoán của bạn.

Hãy làm thử hai thí nghiệm nhỏ ngay bây giờ:

1. **Thí nghiệm 1:** Đổi `package main` thành `package hello`, rồi gõ `go run main.go`.
   - *Kết quả:* Compiler từ chối chạy và báo lỗi: `package command-line-arguments is not a main package`. Compiler khẳng định rằng nó không thể tìm thấy điểm khởi đầu của chương trình nếu gói không mang tên `main`.
...
Triết lý của Go là không khoan nhượng với mã thừa. Bất kỳ thư viện nào được nạp mà không dùng, bất kỳ biến nào được khai báo mà không đọc đều bị coi là lỗi biên dịch. Điều này giúp các dự án hàng triệu dòng mã của Go luôn tinh gọn, sạch sẽ và biên dịch với tốc độ chớp nhoáng.
