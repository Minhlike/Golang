# Chương 18 — Đưa thay đổi ra môi trường vận hành

Sau Chương 17, ta có thể đặt một hiện vật gói phát hành có digest vào trạng thái mong muốn. Nhưng một digest không tự trả lời hiện vật gói phát hành ấy đến từ source nào, đã đi qua test nào, job nào có quyền triển khai, hay rollout xong có nghĩa gì với người dùng. Một pipeline xanh chỉ là tín hiệu về những bước nó đã chạy; nếu các bước không nói rõ contract, màu xanh tạo cảm giác an toàn mạnh hơn bằng chứng thực tế.

Mental model của chương là: **delivery là chuỗi bằng chứng gắn một source revision với một hiện vật gói phát hành bất biến, rồi cho phép promotion theo policy hẹp.** Pipeline không “đưa mã nguồn lên môi trường vận hành”. Nó build một hiện vật gói phát hành, ghi identity của hiện vật gói phát hành, thu evidence cho các gate đã định nghĩa, và chỉ một job có quyền phù hợp mới cập nhật trạng thái mong muốn bằng chính digest ấy. Rollback cũng là một promotion có chủ đích tới hiện vật gói phát hành đã biết, không phải nút hoàn tác cho mọi side effect của hệ thống.

## Một commit xanh vẫn chưa là release

Giả sử một PR pass unit test. Sau đó runner build image, job triển khai dùng tag `:main`, cluster pull image, và rollout hiện `3/3 Available`. Chuỗi này còn nhiều câu hỏi chưa được trả lời: image đang chạy có đúng là image vừa build không; source revision nào đã tạo image; credential triển khai có quyền rộng đến đâu; `3/3` chỉ ra replica available hay chứng minh yêu cầu business thành công; và nếu data migration đã chạy, rollback image có đảo dữ liệu lại không?

Đây không phải lý do để bỏ CI/CD vì “quá phức tạp”. Nó là lý do để mỗi bước hứa một điều nhỏ, kiểm chứng được. Source revision là identity của đầu vào source. hiện vật gói phát hành digest là identity của output build. Test là evidence về behavior mà test bao phủ. Provenance là evidence về quan hệ build mà verifier đã xác nhận. Promotion policy quyết định evidence nào đủ để thay trạng thái mong muốn. Rollout status là observation về bộ điều khiển (controller) và replica, không phải SLO.
...
| --- | --- | --- |
| Commit/revision | Source input đã được chọn. | hiện vật gói phát hành nào thật sự được build. |
| Image digest | Một hiện vật gói phát hành content-addressed cụ thể. | hiện vật gói phát hành đó qua test hay provenance nào. |
| Test pass | Contract của test hiện có đã pass. | Toàn bộ behavior môi trường vận hành, data hay phụ thuộc. |
| Rollout complete | bộ điều khiển (controller) đã đạt điều kiện rollout của tải công việc. | SLO, chi phí, hay business transaction của user. |

Tag dễ đọc vẫn có ích cho con người, nhưng tag là một tên có thể trỏ sang hiện vật gói phát hành khác theo thời gian. Promotion môi trường vận hành nên mang digest mà gate đã xét, không rebuild “cùng commit” ở job sau rồi hy vọng hai output giống nhau. Nếu cần reproducibility cao hơn, build policy phải pin input phù hợp, ghi revision/toolchain và quản lý base image/phụ thuộc; digest chỉ nhận diện output, không tự giải thích output được tạo thế nào.

## Gate không phải nghi thức, mà là policy có người chịu trách nhiệm

Một pipeline tốt không bắt mọi thay đổi chạy toàn bộ suite đắt tiền chỉ vì dashboard thích nhiều ô xanh. Nó đặt gate theo risk và feedback loop. Static check hoặc unit test nhanh có thể chạy sớm trên mọi thay đổi. Integration test có phụ thuộc thật có thể chạy ở stage khác, với hết thời hạn (timeout) và teardown rõ ràng. Build/push chỉ chạy khi source đã qua gate cần thiết. Promotion môi trường vận hành cần identity bất biến, evidence được xác minh, permission hẹp và một đường rollback đã được diễn tập.

Cost guardrail cũng là một contract. Một job integration có thể tạo cluster tạm, registry storage hay egress; phải có owner, hết thời hạn (timeout), concurrency limit, cleanup và giới hạn phạm vi. “Tốn tiền” không tự làm một job sai, còn “CI xanh” không hợp thức hóa resource không có vòng đời. Hãy đo chi phí của stage nặng trước khi tối ưu, giống cách Chương 10 đặt benchmark trước khi đổi mã nguồn.

> **Dừng để dự đoán:** Nếu build job pass nhưng không ghi digest vào promotion record, triển khai job đang chứng minh điều gì khi nó pull `:main`? Nếu security scan báo sạch nhưng image triển khai là output của một build khác, scan đã xét đúng artefact chưa?
...
## Quyền triển khai là capability, không phải biến môi trường tiện tay

CI runner có thể chạy source và action mà workflow cho phép, nên credential triển khai không nên xuất hiện ở mọi job. Với GitHub Actions, `GITHUB_TOKEN` nên bắt đầu bằng quyền đọc tối thiểu rồi tăng quyền tại job thực sự cần. Cloud credential dài hạn được chép vào secret là một boundary rủi ro khác: secret có thể bị dùng nhầm, log redaction không phải một bằng chứng tuyệt đối, và policy ở cloud mới quyết định token có làm được gì.

OIDC là một cách đưa capability về đúng job cần nó. Workflow xin identity token; cloud provider kiểm tra claim/trust condition rồi cấp access token ngắn hạn cho job. Nó không tự cấp quyền triển khai: trust policy vẫn phải giới hạn repository, branch/tag hoặc environment, role và resource. `id-token: write` chỉ cho phép job xin token OIDC, không tự cho phép sửa cloud resource. Đó là lý do “bật OIDC” không thể thay thế review policy IAM.
...
jobs:
  triển khai-môi trường vận hành:
    permissions:
      contents: read
      id-token: write
...
Đoạn YAML chỉ mô tả **ý định phân quyền**, chưa phải workflow triển khai chạy được: nó không định nghĩa cloud role, subject claim, approval rule, digest, action version hay command. Đừng copy snippet rồi thêm secret quyền admin để vượt qua bước trust. Thiết kế đúng phải viết từ resource mà job được phép thay đổi, identity của workflow được cloud tin, và điều kiện nào khiến token bị từ chối.

## Promotion phải giữ identity của hiện vật gói phát hành

Lab không ký image và không gọi registry. Nó giữ lại boundary quyết định: candidate có digest hợp lệ, revision để truy vết, test evidence và provenance evidence; thiếu gate trả một `Decision` từ chối, còn digest sai là input lỗi. Phân biệt hai case này giúp bên gọi không biến configuration sai thành “release bị từ chối bình thường”.
...
Kubernetes triển khai có revision cho thay đổi Pod template và có thể rollout quay về revision trước. Nhưng revision không bao phủ mọi state ngoài Pod template: thay đổi `replicas` không tạo revision, history có thể bị dọn theo `revisionHistoryLimit`, external cấu hình có thể đã đổi, còn database migration và message đã gửi không tự đảo. Vì thế “rollback được” phải được nói thành câu cụ thể hơn: *rollback hiện vật gói phát hành/template nào, trong môi trường nào, với data contract và side effect nào?*
...
~~~text
rollback yêu cầu
  -> chọn hiện vật gói phát hành/revision đã biết
...
Đừng tự động rollback chỉ vì một metric chớp đỏ nếu policy chưa định nghĩa window, ownership và false-positive cost. Nhưng cũng đừng để “manual rollback” nghĩa là một người gõ tag tùy ý lúc sự cố vận hành. Cùng identity, gate và permission boundary của forward delivery phải được giữ khi quay lại.

## Case study: rollout xanh, user vẫn fail

Một release `sha256:abc...` pass unit và integration test, provenance đã verify, rồi được triển khai bằng digest. triển khai báo complete: Pod mới available. Vài phút sau, availability SLI tụt vì cấu hình ở môi trường vận hành trỏ tới điểm cuối phụ thuộc đã retire. Không evidence nào ở đây mâu thuẫn nhau:

- Build/promotion proof nói hiện vật gói phát hành và gate nào đã được dùng.
- triển khai status nói bộ điều khiển (controller) đã thay replica theo policy.
- SLI/log/trace của Chương 16 nói user path đang fail và giúp tìm cohort/cấu hình lệch.

Action đúng không phải kết luận “CI vô dụng” hay “Kubernetes đánh lừa mình”. Team kiểm tra blast radius, candidate cấu hình và data contract; nếu rollback digest cũ khôi phục khả năng phục vụ mà không phá migration, họ promote digest cũ theo runbook. Sau sự cố vận hành, gate cần được sửa ở boundary đã thiếu: configuration validation hoặc integration environment gần môi trường vận hành hơn, không nhất thiết là thêm một security scan vô liên quan.
...
Sau khi làm xong, thử viết một test mới: nếu `TestsPassed` là false nhưng provenance true, `Decision` có được giữ digest hay không? Hãy chọn policy và diễn đạt lý do. Lab hiện chọn không giữ digest trong decision bị từ chối để bên gọi không vô tình lấy identity đó đi triển khai; policy khác chỉ hợp lệ khi được ghi rõ và có test riêng.
...
2. **Local image identity:** Tên/tag cục bộ hoặc Image cấu hình ID (`.Id`) mà
   Docker daemon tạo ra trên máy build.
3. **Manifest digest:** Mã băm nội dung bất biến của OCI Image Manifest
   (`sha256:...`) bao gồm bộ mô tả (descriptor) của mọi layer và cấu hình, được registry và
...
7. **Actual triển khai:** Thao tác cập nhật trạng thái mong muốn của tải công việc bằng
...
Job: verify (test từng module: part16, part18)
      |
      v
Job: build-hiện vật gói phát hành (Docker Buildx metadata)
      |  --> phân biệt Image ID và OCI Manifest Digest
      v
Job: promote-môi trường vận hành (environment: môi trường vận hành)
      |  --> quyền id-token: write cho OIDC
      |  --> chạy promote-gate CLI (kiểm tra candidate)
      |  --> từ chối nếu thiếu verified evidence
      |  --> cập nhật trạng thái mong muốn bằng manifest digest
~~~
...
2. **Quyền mặc định chỉ đọc:** Khai báo `permissions: { contents: read }` ở cấp
   cao nhất. Quyền `id-token: write` chỉ được mở duy nhất ở job triển khai.
...
4. **Không đánh đồng Local Image ID với Manifest Digest:** Khi build cục bộ mà
   chưa push registry, Docker daemon chỉ lưu Image cấu hình ID (`.Id`). Chỉ khi
...
(exit mã nguồn) chuẩn của hệ điều hành:
...
Đặc biệt, trong pipeline CI, khi `promote-gate` trả về mã lỗi (khác 0) do thiếu provenance hoặc trượt test, job quảng bá phải dừng ngay lập tức. Mọi cơ chế nuốt lỗi (như `|| echo` hay `continue-on-error`) để bước triển khai tiếp tục chạy đều là "evidence theater": dựng gate cho có nhưng vẫn âm thầm phát hành mã nguồn không kiểm chứng. Candidate bị gate từ chối đồng nghĩa với việc không có bất kỳ lệnh triển khai nào được phép kích hoạt.
...
condition {
  test     = "StringEquals"
  biến = "token.actions.githubusercontent.com:sub"
  các giá trị   = ["repo:Minhlike/Golang:environment:môi trường vận hành"]
}
~~~

Lưu ý: cấu hình Terraform này là tài liệu và mã nguồn kiểm tra cú pháp
(reviewable mã nguồn). Nó không chứng minh rằng role đã assume thành công, không
chứng minh tài nguyên ECR/App Runner thực tế tồn tại, và không chứng minh lệnh
triển khai đã hoàn tất. Máy hiện tại không cài `terraform` hay `aws`; ta giữ
...
| --- | --- | --- | --- |
| Source Revision | Git commit SHA. | Git commit graph. | Chưa chứng minh mã nguồn biên dịch được. |
| Verification | `go test`, `go vet`, `go test -race`. | Runner CI phụ thuộc graph. | Chỉ bao phủ các ca kiểm thử hiện có. |
| Local Image ID | cấu hình JSON hash (`.Id`). | Docker daemon cục bộ. | Không dùng làm định danh kéo ảnh từ xa. |
...
| Promotion Gate | CLI Go kiểm tra fail-closed. | Admission policy. | Không thay thế được môi trường môi trường vận hành thật. |
| triển khai | Cập nhật trạng thái mong muốn cụm. | Orchestrator bộ điều khiển (controller). | Replica Available không bảo đảm business SLO. |
...
Pipeline, OIDC, attestation, triển khai và rollback không làm software tự an toàn. Chúng làm đường đi của một thay đổi có thể kiểm tra: source nào, hiện vật gói phát hành nào, gate nào, quyền nào, rollout nào và hành động nào khi xấu. Đó là nền để một Go service có thể được phát hành nhiều lần mà không biến mỗi lần phát hành thành một niềm tin mơ hồ.

Từ đây, sách có thể quay vào case study lớn hơn: một service nhận input, chạy work có thời hạn xử lý (deadline), phát signal, được đóng gói, rồi được đưa qua delivery policy và phục hồi có evidence. Mỗi công cụ chỉ được thêm khi câu hỏi vận hành đã tồn tại.
...
3. GitHub Docs. Security for GitHub Actions: hiện vật gói phát hành attestations và triển khai hardening. docs.github.com/en/actions/how-tos/secure-your-work
4. Kubernetes Authors. Deployments: rollout status, revision, rollback và giới hạn của revision history. kubernetes.io/docs/concepts/workloads/các bộ điều khiển (controllers)/triển khai/
